...
@Path("/adherents")
public class DocumentAdherentEndpoint extends DocumentEndpoint {
	private static final String RANG = "rang";

    @Path("/{id}/documents")
    @GET
    @Produces(javax.ws.rs.core.MediaType.APPLICATION_JSON)
    public Response searchDocumentAdherent(
            @HeaderParam(AUTHORIZATION) String authKey,
            @PathParam(ID) Long sAssu,
            ...
            @QueryParam(RANG) Integer rang,
			@QueryParam(TYPE_CODE) String typeCode
			)
            throws GedeException {
		DocumentsFilters filters = new DocumentsFilters(contexte, typeCode);

    	SearchFilteredDocsDto responseSearch;
    	if (rang != null) {
    		 responseSearch = documentService.searchDocStarwebByAssureWithRang(sAssu, contexte, cmroc, envir, page, size, rang, filters);
    	}
    	else {
    		 responseSearch = documentService.searchDocStarwebByAssure(sAssu, contexte, cmroc, envir, page, size, filters);
    	}
